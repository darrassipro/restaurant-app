...
export default function CheckoutScreen() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'cod'>('cod'); // Only COD for now
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      Alert.alert('Erreur', 'Une erreur est survenue lors de la création de votre commande. Veuillez réessayer.');
    }
  };
          ...
          <View className="bg-white rounded-lg shadow-sm p-4 mb-4">
            ...
            {cartItems.map((item) => (
              ...
              >
                ...
                <Text className="font-medium">{formatCurrency(parseFloat(item.dish.price) * item.quantity)}</Text>
              </View>
            ))}
          </View>
