...
export default function ForgotPasswordScreen() {
  ...
  const validateForm = (): boolean => {
    if (!identifier.trim()) {
      setError(
        identifierType === 'email' ? 'Email requis' : 'Numéro de téléphone requis'
      );
      return false;
    }
    ...
    if (
      ...
      !/^\+?\d{10,15}$/.test(identifier.replace(/\s/g, ''))
    ) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Erreur',
        error.data?.message || "Cet utilisateur n'existe pas"
      );
    }
  };
      ...
      <View className="flex-1 bg-white p-6 justify-center">
        <View className="items-center mb-8">
          ...
          <Text className="text-gray-600 text-center mt-2">
            Entrez votre {identifierType === 'email' ? 'email' : 'téléphone'} pour
